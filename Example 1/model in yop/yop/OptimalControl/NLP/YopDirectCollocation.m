...
classdef YopDirectCollocation < handle
    ...
    methods
        ...
        function obj = YopDirectCollocation(controlIntervals, polynomialDegree)
            ...
            % verify if we need to change this in code or just specifying
...
%                     g.dynamics(( dp - w.dt*dx ));
                    if k == 1 && j == 2
                        % JOHaN
                        % Left Border where state is 0, so multiplying with
                        % 0 would give arbitrary derivative
                        zz1 = w.x(k,1); % Will be constrained to zero
                        zz2 = w.x(k,2);
                        % Use average of 0 and next interpolant value
                        g.dynamics(( dp*(zz1(2)+zz2(2))/2 - w.dt*dx ));
                    elseif k == K && j == 2
                        zz1 = w.x(k,1);
                        zz2 = w.x(k,2);
                        g.dynamics(( dp*(zz1(2)+zz2(2))/2 - w.dt*dx ));
                    else
                        % Not border so we can multiply because state
                        % should only be zero at the start and finish
                        zz1 = w.x(k,j);
                        g.dynamics(( dp*zz1(2) - w.dt*dx )); 
                        % REMIND TO FILIPE: your coordinates are differnt,
