...
classdef YopOcp < handle
    ...
    methods % main functionality
        ...
        function solution = solve(obj, varargin)
            ...
            % JOHAN: This setting is overriden later anyway so it makes no
            % difference!
            % Filipe: It seems to have no effect on the polynomial degree
            ip.addParameter('polynomialDegree', 1);
            ...
            solution = obj.optimize(varargin{:});  % It's here where the problem is solved   
        ...
        function build(obj, varargin)
            ...
            ip.addParameter('collocationPoints', 'legendre');
            % JOHAN: Mostly tested with linear, but it appears to work now
            % with arbitrary interpolants
            % FILIPE: It can be set as a parameter in ocp.solve function
            ip.addParameter('polynomialDegree',1); 
            ip.addParameter('controlIntervals', 100*ones(length(obj),5));
            ...
            if length(controlIntervals) < length(obj) % Don't understand
            ...
            for k=1:length(obj)                                
                ...
                obj(k).setObjective; % Mayer and Lagrange terms added
    ...
    methods % Parsing
        ...
        function initialGuess = parseInitialGuess(obj, userInitialGuess)
            ...
            initialGuess = YopNlpVariableInitializer(variables, values); % here
        end
