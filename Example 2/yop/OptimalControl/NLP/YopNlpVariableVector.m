...
classdef YopNlpVariableVector < handle
    ...
    methods
        ...
        function [ubw, lbw] = setBoxConstraints(obj, ocp)
            ...
            % This is where we change the constraint in each interval
            % First do the standard bounds (avoids bounds with functions)
            ...
            % Now look for functional bounds. Evaluate the height of the
            % container stacks along xp.
            % ocp.BoxFinal(1).Constraint{1} = t(tf) is the constraint on
            % length of the loading site.
            % ocp.BoxInitial(2).Constraint{1} is t(0) is the starting
            % position
            for ii = 1:length(ocp.Box)
                cc = ocp.Box(ii).Constraint;
                if isa(ocp.Box(ii).Constraint{1},'function_handle')
                    % ocp.BoxFinal(1).Constraint{1} = t(tf) is the constraint on
                    % length of the loading site. Don't change the order
                    xp0 = ocp.BoxInitial(2).Constraint{1};
                    xpf = ocp.BoxFinal(1).Constraint{1};
                    xp = [xp0:(xpf-xp0)/obj.ControlIntervals:xpf]'+obj.CollocationPoints/obj.ControlIntervals;
                    xp = xp';xp = xp(:);
                    xp = xp(1:end-length(obj.CollocationPoints)+1);
                    f_low = ocp.Box(ii).Constraint{1};
                    lbw(obj.State.Index(3:obj.nx:end)) = f_low(xp);
                end
                if isa(ocp.Box(ii).Constraint{5},'function_handle')
                    xp0 = ocp.BoxInitial(2).Constraint{1};
                    xpf = ocp.BoxFinal(1).Constraint{1};
                    xp = [xp0:(xpf-xp0)/obj.ControlIntervals:xpf]'+obj.CollocationPoints/obj.ControlIntervals;
                    xp = xp';xp = xp(:);
                    xp = xp(1:end-length(obj.CollocationPoints)+1);
                    f_high = ocp.Box(ii).Constraint{5};
                    ubw(obj.State.Index(3:obj.nx:end)) = f_high(xp);
                end
